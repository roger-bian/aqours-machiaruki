...
def run_pipeline():
    ...
    with tempfile.NamedTemporaryFile(suffix='.kml') as new_tmp:
        try:
            ...
            img_urls = cache_images([r['_raw_img_url'] for r in records])
            ...
            raise HTTPException(status_code=422, detail=f'KML structure validation failed: {e}')
        except Exception as e:
            raise HTTPException(status_code=502, detail=f'pipeline processing error: {e}')
